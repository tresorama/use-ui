...
const TYPO = {
  mainNavItem: {
    ...
    textUnderlineOffset: "0.2em" // textTransform: "uppercase",
  ...
  button: {
    ...
    lineHeight: "1",
    letterSpacing: "0.02857em" // transition: "background-color 250ms cubic-bezier(0.4, 0, 0.2, 1) 0ms,box-shadow 250ms cubic-bezier(0.4, 0, 0.2, 1) 0ms,border 250ms cubic-bezier(0.4, 0, 0.2, 1) 0ms",
    // fontFamily: ,
...
const EXTRACTOR_ADDONS = props => {
  const {
    theme
  } = props; // extract special TYPOGRAPHY "style-snippet", to inject ...

  const TYPOprops = theme.TYPO[props.TYPO]; // extract special PRESETS "style-snippet", to inject ...
  ...
  const PRESETSprops = (() => {
    ...
    try {
      ...
      let cssProps = {};
      presetNames.forEach(presetName => cssProps = { ...cssProps,
...
const CONVERTERS = {
  getConverterFunction(cssPropKey) {
    ...
    const {
      MAP
    } = this; // get converter function for this CSS prop...
    ...
    for (const converterName in MAP) {
      if (Object.hasOwnProperty.call(MAP, converterName)) {
        ...
        const propsAllowed = MAP[converterName]; // get convertert if this prop name is included ...
  ...
  MAP: {
    getSize: ["padding", "paddingX", "paddingY", "paddingTop", "paddingRight", "paddingBottom", "paddingLeft", "margin", "marginX", "marginY", "marginTop", "marginRight", "marginBottom", "marginLeft", "borderRadius"],
  ...
  getTextSize: (size, {
    TEXT: T
  }) => T.getFontSizeValue(size),
  getColor: (key, {
    COLOR: C
  }) => C.getColorValue(key),
  getFontFamily: (index, {
    FONT: F
  }) => F.getFontFamilyValue(index),
  getTransitionTime: (a, {
    TRANSITION: T
  }) => T.getTime(a),
  getTransitionTimingFunction: (a, {
    TRANSITION: T
  }) => T.getTiming(a)
};
/* =================================================== 
  ...
  return { ...COLOR,
    ...
    getColorValue(a) {
      try {
        ...
        if (isKey) {
          const colorCode = this[a]; // theme color key is a method ???
...
const buildTEXT = function ({
  ...
  const {
    options,
    ...rest
  } = TEXT; // build keys array ..

  const keys = [...Object.keys(rest)]; // build values array ...
...
const buildBP = function ({
  ...
  return {
    ...
    getWidthValue(a) {
      let key = ""; // a === number => index of key

      if (TYPE.isNumber(a)) key = this.keys[a];else if (TYPE.isString(a)) {
        // a === string in keys =>  name of key
        const foundKey = this.keys.find(_ => a === _);
        if (foundKey) key = foundKey; // a === string not in keys => custom media query
...
const buildTRANSITION = function ({
  TRANSITION
}) {
  return { ...TRANSITION,
    options: {

// js hepers
import { extractPropertiesByCondition, excludePropertiesByCondition, updateSubObjectsOnlyWith, inArray, TYPE, clone } from "../jshelpers/index.js";
...
const isEmptyObject = object => Object.keys(object).length === 0; // BUSINESS LOGIC FUNCTIONS
...
const UI = function (allProps) {
    ...
    const mediaQueriesFast = (() => {
      ...
      Object.keys(mediaQueriesFastRaw).forEach(cssPropName => {
        mediaQueriesFastRaw[cssPropName].forEach((cssPropValue, themeBreakpointIndex) => {
          ...
          const mediaQueryKey = theme.BP.up ? theme.BP.up(themeBreakpointIndex) : fallbackTheme.BP.up(themeBreakpointIndex);
  ...
  const buildUIObject = o => new UI({ ...o,
    theme
  }).getFinalObject();

  const convertNestedPropsWithTheme = object => isEmptyObject(object) ? null : updateSubObjectsOnlyWith(buildUIObject)(object); //    convert directCSS

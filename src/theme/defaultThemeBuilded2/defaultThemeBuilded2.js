...
import styleGuide from "./defaultThemeBuilded2.styleGuide";
import extractAdditionalProps from "./defaultThemeBuilded2.extractor";
import CONVERTERS from "./defaultThemeBuilded2.converter";
...
const buildCOLOR = function ({ COLOR }) {
  return {
    ...
    methodsRunner(colorCode, methodsString) {
      const newColor = COLOR_UTILITIES.runColorMethodsChain(
        colorCode,
        methodsString
      );
      return newColor;
...
const themeBuilder = (styleGuide) => ({
  BP: buildBP(styleGuide),
  COLOR: buildCOLOR(styleGuide),
  SIZE: buildSIZE(styleGuide),
  TEXT: buildTEXT(styleGuide),
  FONT: buildFONT(styleGuide),
  TRANSITION: buildTRANSITION(styleGuide),
  PRESETS: styleGuide.PRESETS,
  TYPO: styleGuide.TYPO,
  // EXTRACTOR
  EXTRACTOR_ADDONS: extractAdditionalProps,
  // CONVERTERS
...
const defaultTheme = themeBuilder(styleGuide);
